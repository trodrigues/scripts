...
# commas.
feeds = ["http://www.rememberthemilk.com/atom/trodrigues/3770634/?tok=eJwNylsKw0AIAMATLbhqVj2Ory2BQiFJ79-O91j0Dl8uNVMxdTmZq0sI0j4mEQaBQmpwGs7DK2Gi24b2mZzjuT51na9v3*N93s8gEVjEY1YAE9Dkrbsg2iwAS9MZADcYxJFl4ejgLNq9sv9bsssJ9QdOmywR"]
...
CACHE_DIRECTORY = "/Users/trodrigues/.rtmcache/"
    ...
    if(len(tasks) == 0):
        ...
        for task in tasks:
            ...
            if(DISPLAY_THE_REAL_LIST_A_TASK_IS_IN_IF_THE_FEED_IS_FOR_A_SMART_LIST and task.has_key("list")):
                s = "\t%s\n%s\t\t%s\t\t%s" % (task["title"], s2, task["due"], task["list"])
                print s.encode("utf-8")
            else:
                s = "\t%s\n%s\t\t%s" % (task["title"], s2, task["due"])
                print s.encode("utf-8")
    print ""
